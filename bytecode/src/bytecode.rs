...
impl Instruction {
    ...
    pub fn stack_effect(&self, jump: bool) -> i32 {
        match self {
            ...
            GetIter => 0,
            Continue { .. } => 0,
            // although the execution of break itself doesn't pop anything, when it unwinds to the
            // loop it's breaking out of it will pop the iterator off of the stack, ergo -1
            Break { .. } => -1,
            Jump { .. } => 0,
